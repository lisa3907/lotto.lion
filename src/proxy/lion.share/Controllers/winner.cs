...
namespace Lion.Share.Controllers
{
    ...
    {
        private DateTime _start_day = DateTime.MinValue;

        public DateTime LottoLion_StartDay
        {
            get
            {
        ...
        public int GetThisWeekSequenceNo()
        {
            return GetSequenceNoByDate(DateTime.Now.Date);
        }

        public int GetSequenceNoByDate(DateTime target_day)
        {
            return (target_day.Subtract(LottoLion_StartDay).Days - 1) / 7 + 1;
        }

        public DateTime GetThisWeekIssueDate()
        {
            return GetIssueDateBySequenceNo(GetThisWeekSequenceNo());
        }

        public DateTime GetNextWeekIssueDate()
        {
            return GetIssueDateBySequenceNo(GetNextWeekSequenceNo());
        }

        public DateTime GetIssueDateBySequenceNo(int sequence_no)
        {
            return LottoLion_StartDay.AddDays((sequence_no - 1) * 7);
        }

        public DateTime GetThisWeekPaymentDate()
        {
            return GetThisWeekIssueDate().AddYears(1);
        }

        public DateTime GetNextWeekPaymentDate()
        {
            return GetNextWeekIssueDate().AddYears(1);
        }

        public DateTime GetPaymentDateBySequenceNo(int sequence_no)
        {
            return GetIssueDateBySequenceNo(sequence_no).AddYears(1);
        }

        private HttpClientHandler __http_handler = null;
        ...
        private async Task<HtmlAgilityPack.HtmlDocument> GetHtmlDocument(int sequnce_no)
        {
            ...
            using (var _client = new HttpClient(__http_handler))
            {
                ...
                var _html_text = await _client.GetStringAsync(_site5);
