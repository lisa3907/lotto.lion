...
namespace Lion.Share.Controllers
{
    ...
    {
        private (short[] lj, short[] ls) GetWinnerPercentage(mWinner[] winners, int sequence_no)
        {
        ...
        public mPercent AnalysisPercentage(AppDbContext ltctx, mWinner[] winners, int sequence_no)
        {
            var _p = GetWinnerPercentage(winners, sequence_no);

            var _percent = new mPercent()
            {
