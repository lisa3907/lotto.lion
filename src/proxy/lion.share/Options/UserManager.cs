...
namespace Lion.Share.Options
{
    public class UserManager
    {
        ...
        /// <returns></returns>
        public async Task<(Claim identity, string mail_address)> GetClaimsIdentityByFacebook(AppDbContext ltctx, ApplicationUser app_user)
        {
        ...
        /// <param name="app_user"></param>
        /// <returns></returns>
        public (Claim identity, string mail_address) GetClaimsIdentityByLoginId(AppDbContext ltctx, ApplicationUser app_user)
        {
            var _result = (identity: (Claim)null, mail_address: "");
        ...
        /// <param name="mail_address">메일 주소</param>
        public bool AddNewMember(AppDbContext ltctx, ApplicationUser app_user)
        {
            var _result = false;

            if (CheckDevice(app_user.device_type, app_user.device_id) == true)
            {
                var _new_member = new mMember
                {
                ...
                ltctx.tb_lion_member.Add(_new_member);
                ltctx.SaveChanges();
        ...
        /// <param name="device_id">장치 식별자</param>
        /// <param name="mail_address">메일 주소</param>
        public bool ReUseMember(AppDbContext ltctx, mMember old_member, ApplicationUser app_user)
        {
            var _result = false;
            ...
            if (CheckDevice(app_user.device_type, app_user.device_id) == true)
            {
                if (old_member.LoginId != app_user.login_id)
                {
                    ...
                    ltctx.tb_lion_member.Remove(old_member);
                    ltctx.tb_lion_member.Add(_new_member);
                }
                else
                {
