...
namespace LottoLion.WebApi.Controllers
{
    ...
    public partial class LottoController : Controller
    {
        ...
        public LottoController(IOptions<JwtIssuerOptions> jwtOptions, IConfiguration configuration, LottoLionContext db_context)
        {
        ...
        public async Task<IActionResult> GetPrizeList(int sequence_no, int limit = 100)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _winners = new List<TbLionWinner>();
                if (sequence_no >= 1 && sequence_no <= WinnerReader.GetThisWeekSequenceNo())
                {
                    _winners = __db_context.TbLionWinner
                                              .Where(w => w.SequenceNo <= sequence_no)
        ...
        public async Task<IActionResult> GetAnalysisBySeqNo(int sequence_no)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _analysis = (TbLionAnalysis)null;
                if (sequence_no >= 1 && sequence_no <= WinnerReader.GetThisWeekSequenceNo())
                {
                    _analysis = __db_context.TbLionAnalysis
                                              .Where(w => w.SequenceNo == sequence_no)
                                              .SingleOrDefault();
        ...
        public async Task<IActionResult> GetAnalysisList(int skip_no, int limit = 100)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _analysis = __db_context.TbLionAnalysis
        ...
        public async Task<IActionResult> GetPercentList(int skip_no, int limit = 100)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _analysis = __db_context.TbLionPercent
                                            .OrderByDescending(p => p.SequenceNo)
                                            .Skip(skip_no)
                                            .Take(limit)
                                            .ToList();
        ...
        private List<TJackpot> GetJackpotList(List<TbLionChoice> choices)
        {
            var _result = new List<TJackpot>();
        ...
        public async Task<IActionResult> GetUserChoices(int sequence_no)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _jackpots = new List<TJackpot>();
                {
        ...
        public async Task<IActionResult> GetChoiceList(int skip_no, int limit = 100)
        {
            return await CProxy.UsingAsync(() =>
            {
                ...
                var _jackpots = new List<TJackpot>();
                {
                    ...
                    if (String.IsNullOrEmpty(_login_id) == false)
                    {
                        _count = __db_context.TbLionChoice
                                                  .Where(c => c.LoginId == _login_id && c.Ranking > 0 && c.Ranking < 6)
                                                  .Count();

                        var _choices = __db_context.TbLionChoice
                                                    .Where(c => c.LoginId == _login_id && c.Ranking > 0 && c.Ranking < 6)
